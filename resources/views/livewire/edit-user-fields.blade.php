...
<div class="row">
    ...
    <div class="col-12">
        <img id="profile-picture"
            ...
            alt="Profile Picture" class="rounded-circle profile-image justify-center mx-auto d-block" height="200px"
            width="200px">
    ...
    <x-adminlte-input name="first_name" label="First name" placeholder="{{ $role }}'s first name"
    ...
    <x-adminlte-input name="fname" value="{{ $user->fname }}" label="Father name *"
        placeholder="{{ $role }}'s Father name" fgroup-class="col-md-3" enable-old-support />
    ...
    <x-adminlte-input name="mname" value="{{ $user->mname }}" label="Mother name *"
        placeholder="{{ $role }}'s Mother name" fgroup-class="col-md-3" enable-old-support />
    ...
    <x-adminlte-input-date name="birthday" value="{{ $user->birthday }}" :config="['format' => 'YYYY/MM/DD']"
        placeholder="Choose {{ $role }}'s Date of Birth..." label="Date of Birth *" fgroup-class="col-md-3"
        autocomplete="off" />
    <x-adminlte-input name="phone" value="{{ $user->phone }}" label="Phone number"
        placeholder="{{ $role }}'s phone number" fgroup-class="col-md-3" enable-old-support />
    <x-adminlte-input name="address" value="{{ $user->address }}" placeholder="Vill/Mohalla" label="Address *"
    ...
    {{-- <div class="col-md-4"> --}}
    <x-adminlte-input name="city" label="District" placeholder="{{ $role }}'s District"
    ...
    <x-adminlte-input name="tehsil" value="{{ $user->tehsil }}" label="Tehsil *"
        placeholder="{{ $role }}'s Tehsil" fgroup-class="col-md-3" enable-old-support />
    <x-adminlte-input class="d-none" value="{{ $user->locality }}" name="locality"
        placeholder="{{ $role }}'s Locality" fgroup-class="col-md-0" enable-old-support />
    <x-adminlte-input name="aadhaar_number" value="{{ $user->aadhaar_number }}"
        placeholder="{{ $role }}'s Aadhaar Number" fgroup-class="col-md-12 no-resize" label="Aadhaar Number *"
        enable-old-support fgroup-class="col-md-3" />
    ...
    <x-adminlte-input value="{{ $user->sub_caste }}" name="sub_caste" label="Sub-Caste *"
        placeholder="{{ $role }}'s Sub-Caste" fgroup-class="col-md-3" enable-old-support />
    <x-adminlte-input value="{{ $user->email }}" readonly="true" name="email" type="email" label="Email address *"
        placeholder="Enter {{ $role }}'s email address" fgroup-class="col-md-3" enable-old-support />
    <x-adminlte-input name="" label=" Password *" placeholder="input a password" readonly="true"
        fgroup-class="col-md-3" type="password" />
    <x-adminlte-input value="{{ $user->password_confirmation }}" name="" readonly="true"
        label="Confirm password *" placeholder="input password again" fgroup-class="col-md-3" type="password" />
    <x-adminlte-input class="d-none" name="last_name" placeholder="{{ $role }}'s last name"
        fgroup-class="col-md-0" enable-old-support />
    <x-adminlte-input class="d-none" name="other_names" value="test"
        placeholder="{{ $role }}'s other names " fgroup-class="col-md-0" enable-old-support />
    ...
    <x-adminlte-input value="{{ $user->previous_school }}" name="previous_school" label="Previos School *"
        placeholder="Previos School" enable-old-support autocomplete="off" fgroup-class="col-md-3" />
    <x-adminlte-input value="{{ $user->bank_name }}" name="bank_name" label="Bank Name *" placeholder="Bank Name"
        enable-old-support autocomplete="off" fgroup-class="col-md-3" />
    <x-adminlte-input value="{{ $user->ifsc }}" name="ifsc" label="IFSC Code*" placeholder="IFSC Code"
        enable-old-support autocomplete="off" fgroup-class="col-md-3" />
    <x-adminlte-input value="{{ $user->holder_name }}" name="holder_name" label="Account Holder Name *"
        placeholder="Account Holder Name" enable-old-support autocomplete="off" fgroup-class="col-md-3" />
    <x-adminlte-input value="{{ $user->acc_no }}" name="acc_no" label="Account Number *"
    ...
    <div class="row">
        ...
        <x-adminlte-input name="sr_no" value="{{ $studentRecord->sr_no }}" label="Unique ID*"
            placeholder="Unique ID" fgroup-class="col-md-3" enable-old-support autocomplete="off" />
        <x-adminlte-input name="roll_no" value="{{ $studentRecord->roll_no }}" label="Roll number *"
        ...
        <x-adminlte-input value="{{ $studentRecord->admission_date }}" name="admission_date"
            label="Admission number *" placeholder="Student's admission number" fgroup-class="col-md-3"
            enable-old-support autocomplete="off" />
        <x-adminlte-input-date value="{{ $studentRecord->admission_date }}" name="admission_date"
